...
_float euclidean_d_s_vec( _float* dvec, pairIF* svec, _int siz )
{
  // dot product of two vectors
  _float prod = 0;
  for( _int i=0; i<siz; i++ )
    {
      ...
      {
        prev_id = svec[i-1].first;
      }
      for(int j=prev_id+1; j<id; j++)
      {
        prod += dvec[j]*dvec[j];
      }
      prod += (dvec[ id ] - val)*(dvec[ id ] - val);
    }
  return 1/prod;
}
...
void kmeans( SMatF* mat, _float acc, VecI& partition, _int K) {
  ...
  while( new_cos - old_cos >= acc ) {
    ...
    //cosines[i][j] = euclidean_d_s_vec( centers[i], mat->data[j], mat->size[j] ); // compute cosine sim
    }
...
void update_svm_scores( Node* node, SMatF* tst_X_Xf, SMatF* score_mat, SMatI* id_type_mat, _float discount, _Septype septype ) {
  ...
  //_float prod = euclidean_d_s_vec( densew, tst_X_Xf->data[ inst ], tst_X_Xf->size[ inst ] ); // dot product
	_float newvalue;
