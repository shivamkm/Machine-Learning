...
################################
        #w_new = np.random.uniform(-1,1,d+1)
        w_new = np.zeros(d+1)
        X_n = np.ones((n,d+1))
        X_n[:,1:] = X 
        B = 10
...
################################
                ...
                regularization_grad = w_new
                rows = np.random.choice(n, B, replace=False)
                X_new = X_n[rows]
                y_new = y[rows]
                #print(np.maximum(1 -  y_new*np.dot(X_new,w_new),0 ))
                temp =  ((n+0.0)/(B+0.0))*(-2*C*np.maximum((1-y_new*(np.dot(X_new,w_new))),0)*y_new)
                #print(temp)
                temp.resize(B,1)
                #print(temp*X_new)
                sq_hinge_grad = np.sum(temp*X_new , 0)
                total_grad = regularization_grad+sq_hinge_grad
                print(total_grad)
                index = np.random.randint(0,d+1)
                w_new[index] = w_new[index] - (lr/ np.sqrt(t))*total_grad[index]
                b = w_new[0]
                w = w_new[1:] 
